...
4. 올린 파일의 **사본을 `output/upload/<#>/` 에 그 이름으로** 둔다(#은 이력
   번호). 같은 이름으로 다시 올려도 앞의 사본을 덮어쓰지 않기 위해서다. 이
   폴더는 git 에 없으니, 다른 PC 에서는 "소스" 칸의 명령으로 다시 만든다
...
| 2 | 2026-09-23 | `v8.18-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` | 19,599,557 | 502 | 현재 코드로 v8.18 빌드 → dedupe → `upload/2/` 에 이 이름으로 복사 |
| 3 | **올리기 전** | `v8.20-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` (#2 와 같은 이름) | 16,517,551 | 502 | v8.20 빌드 → dedupe → `upload/3/` 에 복사. GoodNotes 바둑판 렌더링 + 도트 그리드 수정 |
