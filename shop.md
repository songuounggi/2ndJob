...
4. **파일 규칙은 하나다** (2026-09-24 확정):
   ...
   - **마켓 판**은 `output/upload/<Etsy 이름>` -- 지금 Etsy 에 올라가 있는 파일.
     **최종 확인 전에는 절대 건드리지 않는다**
   - **보관**은 `output/upload/<#>/<Etsy 이름>` -- 최종 확인을 거쳐 Etsy 에 올린 판을
     이력 번호대로 쌓는다. 구매자가 받은 파일을 그대로 꺼내 보기 위해서다
   - 새 판을 올리는 날: 이력표에 날짜를 적고 → `upload/<#>/` 에 보관 → `upload/`
     바로 아래 마켓 판을 교체
...
| 2 | 2026-09-23 | `v8.18-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` | 19,599,557 | 502 | 현재 코드로 v8.18 빌드 → dedupe → `upload/2/` 에 보관 |
| 3 | **올리기 전** | `v8.20-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` (#2 와 같은 이름) | 16,517,551 | 502 | v8.20 빌드 → dedupe. 최종 확인 후 `upload/3/` 보관 + 마켓 판 교체. GoodNotes 바둑판 렌더링 + 도트 그리드 수정 |

**현재 판매 중: #2.** 1번은 교체되어 내려갔다. #3 은 v8.19 로 준비했다가
사용자가 iPad 에서 도트 그리드 결함을 찾아 v8.20 으로 바꿨다(2026-09-24). Etsy 교체 대기. **올리면 날짜를 채우고 이 줄을 고친다.**

#3 교체 후 확인할 것 — **기존 구매자가 새 파일을 받는가**(5-5절 미확인 항목):
첫 구매자 주문의 `View files` 크기가 **15.75 MB** 면 새 파일, **18.69 MB** 면
옛 파일이 그대로 묶여 있는 것이다. (Etsy 는 1024 단위로 표시한다:
19,599,557 B 가 18.69 MB 로 보였다. 16,517,551 B 는 15.75 MB 다.)
...
|---|---|---|---|---|---|---|
| 1 | 2026-09-23 22:39 | 브라질 | USD 10.19 (정가 16.99) | `LAUNCH40` | PDF 이력 **#2** (`...-502-pages.pdf`, Etsy 표시 18.69 MB) | 9/23 받음 |
