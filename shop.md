...
4. 올린 파일의 **사본을 `output/upload/` 에 그 이름으로** 둔다. 이 폴더는
   git 에 없으니, 다른 PC 에서는 "소스" 칸의 명령으로 다시 만든다
...
| 2 | 2026-09-23 | `v8.18-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` | 19,599,557 | 502 | 현재 코드로 v8.18 빌드 → dedupe → 이 이름으로 복사 (`SETUP.md` 5단계) |

**현재 판매 중: #2.** 1번은 교체되어 내려갔다.
