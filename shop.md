...
| 3 | **올리기 전** | `v8.19-undated` | `ADHD-Wellness-Planner-Undated-502-pages.pdf` (#2 와 같은 이름) | 16,091,246 | 502 | v8.19 빌드 → dedupe → `upload/3/` 에 복사. GoodNotes 바둑판 렌더링 수정 |

**현재 판매 중: #2.** 1번은 교체되어 내려갔다. #3 은 사용자가 iPad 에서
확인(2026-09-24), Etsy 교체 대기. **올리면 날짜를 채우고 이 줄을 고친다.**

#3 교체 후 확인할 것 — **기존 구매자가 새 파일을 받는가**(5-5절 미확인 항목):
첫 구매자 주문의 `View files` 크기가 **15.35 MB** 면 새 파일, **18.69 MB** 면
옛 파일이 그대로 묶여 있는 것이다. (Etsy 는 1024 단위로 표시한다:
19,599,557 B 가 18.69 MB 로 보였다. 16,091,246 B 는 15.35 MB 다.)
