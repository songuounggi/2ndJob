...
| 지역 | South Korea |
| 상품 파일 | **아래 업로드 이력의 맨 아래 줄이 현재 판매 중인 파일** |
| 리스팅 이미지 | 〃 |

---

## 0-1. Etsy 업로드 이력 — **Etsy 에 무언가 올릴 때마다 한 줄 추가**

2026-09-24 에 만들었다. 그 전에는 이 기록이 없어서, 다른 PC 에서 "지금 Etsy
에 올라간 파일이 뭐냐"에 답하지 못했다.

**규칙**
1. Etsy 에 파일·이미지를 올리거나 바꾸면 **그날 바로** 한 줄 추가한다. 나중에 몰아서 적지 않는다
...
4. **파일 규칙은 하나다** (2026-09-24 확정):
   ...
   - 새 판을 올리는 날: 이력표에 날짜를 적고 → `upload/<버전>/` 을 만들어 넣는다
   - `python scripts/check_upload.py` 로 표·작업본·보관본·마켓 판을 대조한다
5. 모르는 칸은 비우지 말고 **`미확인`** 이라고 적는다. 짐작으로 채우지 않는다

### 디지털 파일 (PDF)

| # | 날짜 | 버전 | Etsy 에 보이는 파일 이름 | 바이트 | 페이지 | 소스 (다른 PC 에서 다시 만드는 법) |
|---|---|---|---|---|---|---|
...
**Etsy 는 화면마다 MB 단위가 다르다.** 리스팅 편집 화면은 1000 단위(16,517,551 B →
`16.51 mb`), 주문의 `View files` 는 1024 단위(19,599,557 B → `18.69 MB`).

### 교체 후 기존 구매자는 어느 파일을 받는가 — **옛 파일 그대로** (2026-09-24 확정)

- 교체 직후 첫 구매자 주문의 `View files` 가 `18.69 MB`(v8.18) 그대로였다
- 셀러 커뮤니티: 리스팅 파일을 바꿔도 **기존 구매자는 구매 당시 파일만** 받는다.
  Etsy 는 기존 구매자에게 알림도 보내지 않는다
- **메시지 첨부로 플래너를 보내지 않는다 -- Etsy 가 링크를 전부 지운다** (2026-09-24 실측).
  v8.20 을 첨부해 보냈더니 받는 쪽 파일이 링크 5,893 → **0**, 16.5 → 15.2MB,
  Producer `Skia/PDF` → `pdfcpu`. Etsy 서버가 다시 저장하면서 링크 주석을 버린다.
  탭이 핵심인 상품이라 치명적이다. (Etsy Help 는 .pdf 첨부가 된다고만 한다)
- 외부 링크(Drive 등)는 Etsy 메시지 규정 위반이다
- **그래서 기존 구매자에게 새 판을 링크째 줄 방법은 지금 없다.** 구매 파일은
  Purchases → Download 에서 계속 받을 수 있고, 옛 판도 링크는 온전하다

**앞으로 파일을 교체하면:** 기존 구매자에게 파일을 보내지 않는다. 옛 판에 기능
결함(링크 깨짐 등)이 있을 때만 방법을 따로 찾는다.
...
첫 구매자 감사 메시지 초안에 **"탭이 오른쪽에 있다"** 고 썼다. 실제로는
왼쪽이다. 파일을 보지 않고 짐작으로 썼고, 사용자가 읽다가 잡았다. 같은
초안에 "iPad GoodNotes 에서 가장 잘 된다"는, 리스팅 설명(안드로이드·윈도우도
된다)과 어긋나는 문장도 있었다.

그래서 메시지·답장·리스팅 문구처럼 **구매자가 읽는 글**은 초안과 함께
**문장별 근거표**를 낸다:

| 문장 | 확인한 곳 | 결과 |
|---|---|---|
| 상품에 관한 사실 (위치·이름·개수·크기) | 판매본 HTML/PDF 에서 직접 뽑는다 | ✅ / ⚠️ |
| 호환성·기능 약속 | `listing.md` 본문. **리스팅에 없는 약속은 새로 하지 않는다** | |
| 숍 사실 (몇 번째 구매자 등) | 이 파일의 판매 표 | |

실기기에서 확인하지 못한 것은 "미확인"이라고 적는다.

### 판매 — 누가 어느 파일을 받았나

파일을 고쳐 다시 올리면 **이 표를 보고 옛 파일을 받은 구매자에게 알린다**
(5-5절: 교체 후 기존 구매자가 무엇을 받는지 Etsy 가 밝히지 않는다).

구매자 이름·주문번호는 **적지 않는다** — 저장소가 공개일 수 있다. 찾을 때는
Shop Manager → Orders → `Completed` 탭(디지털 주문은 `New` 가 아니라 여기로
바로 간다)에서 날짜로 찾는다.

| # | 주문 시각 (Etsy 표시) | 나라 | 결제액 | 쿠폰 | 받은 파일 | 다운로드 |
|---|---|---|---|---|---|---|
| 1 | 2026-09-23 22:39 | 브라질 | USD 10.19 (정가 16.99) | `LAUNCH40` | **v8.18** (주문에 묶인 파일, 18.69 MB, 링크 정상). 9/24 v8.20 을 메시지로 첨부했으나 Etsy 가 링크를 지워 **구매 파일을 쓰라고 정정 안내** | 9/23 받음 |
...
### 파일을 교체하면 기존 구매자는 어느 버전을 받는가 — **옛 파일 그대로** (0-1절, 2026-09-24)
...
## 6. v8.18 교체 — **완료** (2026-09-23)

Etsy 에서 팔리는 파일은 이제 **v8.18-undated (502p)** 다. 다섯 단계를
모두 마쳤고, 실제 페이지에서 502/61 로 반영된 것을 확인했다.

> 확인할 때 **첫 요청이 캐시된 옛 페이지를 줬다.** 다시 받아야 새 내용이
> 나온다. 발행 직후 "안 바뀌었다"고 판단하지 말 것.
