...
### 교체 후 기존 구매자는 어느 파일을 받는가 — 관찰 중

| 확인 시각 | 첫 구매자 주문 `View files` | 해석 |
|---|---|---|
| 2026-09-24 교체 직후 | `18.69 MB` (v8.18) | 아직 옛 파일. 반영 지연인지, 주문마다 파일이 고정되는지 **미확인** |
| 2026-09-25 다시 볼 것 | | `15.75`/`16.51` 이면 새 파일로 바뀐 것, `18.69` 그대로면 주문에 고정 |

결론이 나면 5-5절의 "확인 못 했다"를 고친다. 첫 구매자에게는 업데이트 안내를
**이 결론 뒤에** 보낸다 -- 받을 방법이 정해지기 전에 알리면 혼란만 준다.
