...
|---|---|---|
| `ADHD-Year-Planner-2026-mon.pdf` | 598 | 7.7MB |
...
## 설명 (Description) — 한 문단 = 한 줄 (Etsy 가 다시 접는다)
...
## 가격 — **확정 (2026-09-25, 사용자: B안)**

| 항목 | 값 |
|---|---|
| Etsy 에 입력하는 **정가** | **$19.99** |
| 런칭 세일 | **35%** (Etsy Sales and discounts 에서 설정) |
| **구매자가 실제로 내는 금액** | **$12.99** |
| 세일 기간 | 미정 -- 올릴 때 정한다 (상품 1 은 출시 후 약 한 달) |

금액은 미국 구매자 기준. 한국에서 Etsy 를 보면 VAT 10% 가 붙어 보인다(`shop.md` 5-0).

근거: 매출 1위 날짜형 경쟁작 Manifestable 2026-2027 이 세일 없이 $10.99. 상품 1 은 $16.99 − 40% = $10.19.
우리는 PDF 4개(2년 × 월·일 시작)라 그보다 조금 위. 정가를 걸고 세일하면 상품 페이지에
할인 표시가 붙는다(상품 1 과 같은 이유).
...
도구·월 계획·Brain weather·월 리뷰·분기·노트 476쪽은 같다. 미국은 보통 일요일 시작, 한국·유럽은 월요일 시작.

---

## 대조표 — 문장 → 확인한 곳 → 결과

`python scripts/p3/check_listing_p3.py` 가 ✔ 항목을 네 파일 모두에서 잰다. △ 는 실기기 미확인.
...
| Tomorrow ↔ yesterday links | 일간마다 `Tomorrow` → 다음 날, `From yesterday` → 전날 (time links broken 0) | ✔ |
...
## 리스팅 이미지 — **시안 10장** `output/prod3/listing/draft-v0.13/` (사용자 확정 전)
