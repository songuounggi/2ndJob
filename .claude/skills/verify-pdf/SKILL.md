...
## 3-1. 뷰어 위험·속도·용량 (생략 금지, 2026-09-24 추가)

```bash
python scripts/check_render.py <대상파일>
```

`FAILURES: 0` 이어야 한다. PC 렌더(1~2단계)가 멀쩡해도 iPad GoodNotes 에서
바둑판처럼 느리게 그려지거나(그라데이션·반투명) 도트 그리드가 4배로 벌어진
(이미지 타일 패턴) 사례가 있다 -- 둘 다 이 단계 없이 출시됐다. 기준과 고치는
법은 `RELEASE.md` 1·2절.

**이 스킬을 통과해도 끝이 아니다.** Etsy 에 올리기 전에 `RELEASE.md` 1-3 의
iPad 실기기 체크리스트를 사용자에게 페이지 번호와 함께 드린다.
