...
PDF = os.path.join(ROOT, 'output', 'prod1', 'planner_v8.10-undated-FINAL.pdf')
HTML = os.path.join(ROOT, 'src', 'planner_v8.10-undated.html')
...
checks = [
    ('pages', len(r.pages), 494),
    ('bytes', nbytes, '< 20,000,000'),
    ('MB (decimal)', round(nbytes / 1_000_000, 2), '< 20'),
    ('destinations', len(nd), 493),
    ('link annots', links, 5805),
