...
VER = "v0.7"
DRAFT = "draft-v0.9"   # v0.8: 07·08 페이지가 작고 아래가 비었다 -> 크게 겹쳐 펼침 / v0.7: 10장으로(사용자: 기존 두 상품 10장) -- 07 한 달, 08 12월, 09 2탭, 10 어디서나 / v0.6: 안전 영역 검사가 06 마지막 배지(오른쪽 1754px)에서 멈춤 -> 배지를 표지 안쪽으로 (v0.6 폴더는 01-05 만 있다) / v0.5: 배지 5도, 안전 영역(글자가 왼쪽 120px 에서 시작 -> 검색 목록에서 잘림) / v0.4: 15도는 너무 기울었다 -> 8도(사용자) / v0.3: 배지 시계방향 15도(사용자) / v0.1: 05 썸네일 3줄이 아래로 잘림, 06 아래가 비었다 / v0.2: 06 표지 네 장이 멀리서 구분 안 됨 -> 배지
...
.fan .tag{{position:absolute;left:18px;bottom:20px;background:{PAPER};border:2px solid {N700};border-radius:999px;padding:6px 20px;font-size:26px;letter-spacing:.08em;text-transform:uppercase;color:{INK};white-space:nowrap}}
...
def fan(items, h, step, drop):
    ...
    out = "".join(f'<div class="fp" style="left:{i * step}px;top:{i * drop}px;height:{h}px;width:{w}px;z-index:{i}">'
                  f'<img src="_pages/{k}.png"><span class="tag">{c}</span></div>' for i, (k, c) in enumerate(items))
...
SHOTS = {
    ...
    "07_month": f"""<div class="k">Every month · four pages that work together</div>
      ...
      {fan([("m3", "Calendar"), ("mp3", "Plan"), ("bw3", "Brain weather"), ("mr3", "Review")], 900, 268, 18)}""",
