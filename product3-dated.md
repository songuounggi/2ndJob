...
**사용자 확인 (2026-09-26, iPad GoodNotes):** 배경 2× / 1.5× 비교 -- "2× 도 속도 이슈 없다" → **2× 유지 확정.** 렌더 B 미달은 실기기 확인으로 넘긴다.
...
## 공용 검수 계획 (2026-09-26 사용자 확정, 진행 중)

**모든 상품은 공용 검사를 통과해야 올린다.** 마무리 때 한 번에 정리한다:
1. `scripts/qa_pdf.py` 하나로 합친다 -- 링크 · PDF 겹침 · 뷰어 위험/속도/용량(`check_render` 호출) · 폰트 ·
   누르는 영역 · 줄 간격 · 라벨 간격. 줄 간격·라벨은 지금 오탐이 많다(v0.12 에서 459쪽) -- "글자 없는 연속 쓰기 줄" 묶음과
   "그 묶음 바로 위 제목 줄"만 재도록 고친다. **검증: v0.10 에서 걸리고 v0.12 에서 0.** 상품 1·2 판매본에도 돌린다
2. 상품별로 남길 것: 잘림·넘침(HTML 구조), `check_lines.py`(상품 1·2), 리스팅 대조
3. 빠진 것 취합, 불필요한 것 버림(`audit_*_p3` 중 공용으로 옮겨진 것)
4. **검수 담당 에이전트** `.claude/agents/` -- 반드시 `qa_pdf.py`·`check_render.py` 를 먼저 돌리고(FAIL 이면 멈춤),
