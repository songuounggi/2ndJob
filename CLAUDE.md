...
| 파일 | 담는 것 |
|---|---|
| **`PROCESS.md`** | **작업 절차서·검수 절차서.** 시장 조사 → 기획서 → 디자인 → 제작 → 검수(기획 대조·디자인·논리·수치) → 직접 써 보기 → iPad → 출시. **모든 상품이 이 순서를 따른다** |
...
### 모든 상품은 `PROCESS.md` 의 검수 5-1~5-5 를 통과해야 올린다 (2026-09-26 사용자 확정)

**5-1 기획서 대조를 빼먹지 않는다** -- 상품 3 에서 몇 시간 검수하면서 한 번도 안 했다.
공용 PDF 검사 `scripts/qa_pdf.py` 는 작업 중이다(줄 간격·라벨 오탐 정리 뒤 확정).

상품 3 검수에서 사용자가 줄 간격·라벨 붙음·겹침을 수십 건 찾았다. 전부 **검사가 없어서** 사람 눈에 맡겨진
것이었다. 빨리 여러 상품을 만들수록 이런 결함이 는다 -- 그래서 검사를 모든 상품에 똑같이 건다.
브라우저 화면이 아니라 **완성된 PDF 를 잰다**(화면과 PDF 배치가 달랐던 사례: 상품 3 v0.11 mailbox).
검사가 FAIL 이면 사용자에게 보내지 않는다. 의도한 예외는 상품 문서에 적고 `--allow` 로 넘긴다.
